        ...
        <el-table :data="tableList" border stripe :height="tableHeight" size="small">
            ...
            <el-table-column prop="payPowerStatus" label="缴费状态">
                <template slot-scope="scope">
                    <el-tag type="danger" size="small" v-if="scope.row.payPowerStatus == '0'">未交费</el-tag>
                    <el-tag type="success" size="small" v-if="scope.row.payPowerStatus == '1'">已交费</el-tag>
                </template>
            </el-table-column>
        </el-table>
        <!-- 分页 -->
...
import {getMyPowerFeeApi} from '@/api/myFee'
    export default {
        ...
        methods: {
            ...
            async getList(){
                let res = await getMyPowerFeeApi(this.parm);
                //console.log(res);
                if(res && res.code == 200){
