  ...
  <div class="navbar">
    ...
    >
      <template slot="content">
        <el-form :model="parm" ref="parmRef" :rules="rules" label-width="80px" :inline="true" size="small">
...
export default {
  ...
  methods: {
    //确认弹窗
    onConfirm(){
      this.$refs.parmRef.validate(async(valid) =>{
        if (valid) {
          let res = await resetPasswordApi(this.parm);
          if (res && res.code == 200) {
    ...
    //重置密码
    async resetPassword(){
      let confirm = await this.$myconfirm('确定要重置密码吗?');
      if(confirm){
